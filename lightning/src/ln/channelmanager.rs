				...
				L::Target: Logger,
{
	...
	#[cfg(any(test, feature = "_test_utils"))]
	pub(super) per_peer_state: FairRwLock<HashMap<PublicKey, Mutex<PeerState<<K::Target as KeysInterface>::Signer>>>>,
        ...
        L::Target: Logger,
{
	...
	pub fn create_channel(&self, their_network_key: PublicKey, channel_value_satoshis: u64, push_msat: u64, user_channel_id: u128, override_config: Option<UserConfig>) -> Result<[u8; 32], APIError> {
		...
		if let None = per_peer_state.get(&their_network_key) {
			return Err(APIError::APIMisuseError { err: format!("Can't find a peer matching the passed counterparty node_id {}", their_network_key) });
		}

		let peer_state_mutex = per_peer_state.get(&their_network_key).unwrap();
	...
	fn list_channels_with_filter<Fn: FnMut(&(&[u8; 32], &Channel<<K::Target as KeysInterface>::Signer>)) -> bool + Copy>(&self, f: Fn) -> Vec<ChannelDetails> {
		let mut res = Vec::new();
...
#[cfg(not(test))]
	...
	fn claim_funds_from_hop<ComplFunc: FnOnce(Option<u64>) -> Option<MonitorUpdateCompletionAction>>(&self,
		...
		per_peer_state_lock: RwLockReadGuard<HashMap<PublicKey, Mutex<PeerState<<K::Target as KeysInterface>::Signer>>>>,
	...
	fn claim_funds_internal(&self, channel_state_lock: MutexGuard<ChannelHolder>, per_peer_state_lock: RwLockReadGuard<HashMap<PublicKey, Mutex<PeerState<<K::Target as KeysInterface>::Signer>>>>, source: HTLCSource, payment_preimage: PaymentPreimage, forwarded_htlc_value_msat: Option<u64>, from_onchain: bool, next_channel_id: [u8; 32]) {
		match source {
			...
			HTLCSource::PreviousHopData(hop_data) => {
				...
				let res = self.claim_funds_from_hop(channel_state_lock, per_peer_state_lock, hop_data, payment_preimage,
	...
	fn internal_update_fulfill_htlc(&self, counterparty_node_id: &PublicKey, msg: &msgs::UpdateFulfillHTLC) -> Result<(), MsgHandleErrInternal> {
		...
		self.claim_funds_internal(channel_lock, per_peer_state, htlc_source, msg.payment_preimage.clone(), Some(forwarded_htlc_value), false, msg.channel_id);
		Ok(())
	}
	...
	fn internal_channel_update(&self, counterparty_node_id: &PublicKey, msg: &msgs::ChannelUpdate) -> Result<NotifyOption, MsgHandleErrInternal> {
		...
		let per_peer_state = self.per_peer_state.read().unwrap();
		if let None = per_peer_state.get(&chan_counterparty_node_id) {
			return Ok(NotifyOption::SkipPersist)
		}
		let peer_state_mutex = per_peer_state.get(&chan_counterparty_node_id).unwrap();
		let mut peer_state_lock = peer_state_mutex.lock().unwrap();
		let peer_state = &mut *peer_state_lock;
				...
				match monitor_event {
					MonitorEvent::HTLCEvent(htlc_update) => {
						if let Some(preimage) = htlc_update.payment_preimage {
							...
							self.claim_funds_internal(self.channel_state.lock().unwrap(), self.per_peer_state.read().unwrap(), htlc_update.source, preimage, htlc_update.htlc_value_satoshis.map(|v| v * 1000), true, funding_outpoint.to_channel_id());
						} else {
        ...
        L::Target: Logger,
{
	fn read<R: io::Read>(reader: &mut R, mut args: ChannelManagerReadArgs<'a, M, T, K, F, L>) -> Result<Self, DecodeError> {
		...
		let mut peer_channels: HashMap<PublicKey, HashMap<[u8; 32], Channel<<K::Target as KeysInterface>::Signer>>> = HashMap::with_capacity(cmp::min(channel_count as usize, 128));
		...
		let mut per_peer_state = HashMap::with_capacity(cmp::min(peer_count as usize, MAX_ALLOC_SIZE/mem::size_of::<(PublicKey, Mutex<PeerState<<K::Target as KeysInterface>::Signer>>)>()));
	...
	fn check_unkown_peer_error<T>(res_err: Result<T, APIError>, expected_public_key: PublicKey) {
		match res_err {
			Err(APIError::APIMisuseError { err }) => {
				assert_eq!(err, format!("Can't find a peer matching the passed counterparty node_id {}", expected_public_key));
			},
			Ok(_) => panic!("Unexpected Ok"),
