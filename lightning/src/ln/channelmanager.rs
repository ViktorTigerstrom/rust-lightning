        ...
        L::Target: Logger,
{
	...
	pub fn create_channel(&self, their_network_key: PublicKey, channel_value_satoshis: u64, push_msat: u64, user_channel_id: u128, override_config: Option<UserConfig>) -> Result<[u8; 32], APIError> {
		...
		match per_peer_state.get(&their_network_key) {
			None => return Err(APIError::APIMisuseError { err: format!("Can't find a peer matching the passed counterparty node_id {}", their_network_key) }),
...
#[cfg(test)]
	...
	fn channel_monitor_updated(&self, funding_txo: &OutPoint, highest_applied_update_id: u64, counterparty_node_id: Option<&PublicKey>) {
		...
		let (mut pending_failures, finalized_claims, counterparty_node_id) = {
			...
			let mut channel = {
				if let Some(peer_state_mutex) = per_peer_state.get(&counterparty_node_id_opt.unwrap()) {
