        ...
        L::Target: Logger,
{
	...
	fn list_channels_with_filter<Fn: FnMut(&(&[u8; 32], &Channel<Signer>)) -> bool + Copy>(&self, f: Fn) -> Vec<ChannelDetails> {
		let mut res = Vec::new();
		{
			...
			// ChannelMonitor handling. Therefore reallocations may still occur, but is unlikely as
			// the `channel_state.short_to_id` map often contains 2 entries for the same channel.
