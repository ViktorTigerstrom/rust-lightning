        ...
        L::Target: Logger,
{
	...
	pub fn create_channel(&self, their_network_key: PublicKey, channel_value_satoshis: u64, push_msat: u64, user_channel_id: u64, override_config: Option<UserConfig>) -> Result<[u8; 32], APIError> {
		...
		} else { return Err(APIError::APIMisuseError { err: "Can't find a peer with a node_id matching the passed their_network_key".to_owned() }); }
		...
		let result: Result<(), _> = loop {
			...
			if let Some(peer_state_mutex) = per_peer_state.get(counterparty_node_id) {
				...
				match peer_state.channel_by_id.entry(channel_id.clone()) {
					...
					hash_map::Entry::Vacant(_) => return Err(APIError::ChannelUnavailable{err: "No such channel for the passed counterparty_node_id".to_owned()})
				}
			} else {
		...
		let mut chan = {
			...
			if let Some(peer_state_mutex) = per_peer_state.get(peer_node_id) {
				...
				if let hash_map::Entry::Occupied(chan) = peer_state.channel_by_id.entry(channel_id.clone()) {
					...
					return Err(APIError::ChannelUnavailable{ err: "No such channel for the passed counterparty_node_id".to_owned() });
				}
			} else {
...
#[cfg(test)]
	...
	fn internal_funding_created(&self, counterparty_node_id: &PublicKey, msg: &msgs::FundingCreated) -> Result<(), MsgHandleErrInternal> {
		...
		if let Some(peer_state_mutex) = peer_state_lock.get(counterparty_node_id) {
			...
			return Err(MsgHandleErrInternal::send_err_msg_no_close(format!("Can't find a peer with a node_id matching the passed counterparty_node_id {}", counterparty_node_id), funding_msg.channel_id))
		}
		Ok(())
	}
	...
	fn internal_announcement_signatures(&self, counterparty_node_id: &PublicKey, msg: &msgs::AnnouncementSignatures) -> Result<(), MsgHandleErrInternal> {
		...
		if let Some(peer_state_mutex) = per_peer_state.get(counterparty_node_id) {
			...
			match peer_state.channel_by_id.entry(msg.channel_id) {
				...
				hash_map::Entry::Vacant(_) => return Err(MsgHandleErrInternal::send_err_msg_no_close("Failed to find corresponding channel for the peer with the passed counterparty_node_id".to_owned(), msg.channel_id))
			}
		} else {
			return Err(MsgHandleErrInternal::send_err_msg_no_close(format!("Can't find a peer with a node_id matching the passed counterparty_node_id {}", counterparty_node_id), msg.channel_id));
		}
