...
fn closure_test<'a, 'b: 'a, 'c: 'b>(node: &Node<'a, 'b, 'c>, closing_node_id: PublicKey, closed_channel_id: [u8; 32]){
	...
	match close_msg_ev[close_msg_ev.len() - 1] {
...
#[test]
fn test_function_calls_with_unkown_counterparty_node_2() {
	use bitcoin::secp256k1::ffi::Signature as FFISignature;
	use bitcoin::secp256k1::ecdsa::Signature;
	let chanmon_cfg = create_chanmon_cfgs(4);
	let node_cfg = create_node_cfgs(4, &chanmon_cfg);
	let node_chanmgr = create_node_chanmgrs(4, &node_cfg, &[None, None, None, None]);
	let nodes = create_network(4, &node_cfg, &node_chanmgr);

	let unkown_public_key = PublicKey::from_secret_key(&Secp256k1::signing_only(), &SecretKey::from_slice(&[42; 32]).unwrap());

	// Boilerplate channel code to produce all the node msgs we need for the test
	...
	nodes[1].node.handle_funding_created(&nodes[0].node.get_our_node_id(), &funding_created_msg);
	let funding_signed = get_event_msg!(nodes[1], MessageSendEvent::SendFundingSigned, nodes[0].node.get_our_node_id());
	let channel_id = funding_signed.channel_id;
	nodes[0].node.handle_funding_signed(&nodes[1].node.get_our_node_id(), &funding_signed);
	check_added_monitors!(nodes[0], 1);
	check_added_monitors!(nodes[1], 1);
	let conf_height = core::cmp::max(nodes[0].best_block_info().1 + 1, nodes[1].best_block_info().1 + 1);
	...
	let nodes_0_channel_ready = get_event_msg!(nodes[0], MessageSendEvent::SendChannelReady, nodes[1].node.get_our_node_id());
	let nodes_1_channel_ready = get_event_msg!(nodes[1], MessageSendEvent::SendChannelReady, nodes[0].node.get_our_node_id());
	nodes[0].node.handle_channel_ready(&nodes[1].node.get_our_node_id(), &nodes_1_channel_ready);
	nodes[1].node.handle_channel_ready(&nodes[0].node.get_our_node_id(), &nodes_0_channel_ready);
	let nodes_0_announcement_sigs = get_event_msg!(nodes[0], MessageSendEvent::SendAnnouncementSignatures, nodes[1].node.get_our_node_id());
	let nodes_1_announcement_sigs = get_event_msg!(nodes[1], MessageSendEvent::SendAnnouncementSignatures, nodes[0].node.get_our_node_id());
	nodes[0].node.handle_announcement_signatures(&nodes[1].node.get_our_node_id(), &nodes_1_announcement_sigs);
	nodes[1].node.handle_announcement_signatures(&nodes[0].node.get_our_node_id(), &nodes_0_announcement_sigs);
	let nodes_0_events = nodes[0].node.get_and_clear_pending_msg_events();
	let nodes_0_channel_update = match nodes_0_events[0] {
		MessageSendEvent::BroadcastChannelAnnouncement { ref update_msg, .. } => {
			update_msg.clone()
		},
		_ => panic!("Unexpected event"),
	};
	let nodes_1_events = nodes[1].node.get_and_clear_pending_msg_events();
	let (announcement, nodes_1_channel_update) = match nodes_1_events[0] {
		MessageSendEvent::BroadcastChannelAnnouncement { ref msg, ref update_msg } => {
			(msg, update_msg)
		},
		_ => panic!("Unexpected event"),
	};
	update_nodes_with_chan_announce(&nodes, 0, 1, &announcement, &nodes_0_channel_update, &nodes_1_channel_update);
	nodes[0].node.peer_disconnected(&nodes[1].node.get_our_node_id(), false);
	nodes[1].node.peer_disconnected(&nodes[0].node.get_our_node_id(), false);
	nodes[0].node.peer_connected(&nodes[1].node.get_our_node_id(), &msgs::Init { features: InitFeatures::empty(), remote_network_address: None });
	let reestablish_1 = get_chan_reestablish_msgs!(nodes[0], nodes[1]);
	...
	nodes[1].node.get_and_clear_pending_msg_events();
	nodes[0].node.close_channel(&nodes_0_channel_ready.channel_id, &nodes[1].node.get_our_node_id()).unwrap();
	...
	let (_nodes_0_update, closing_signed_2_node_0) = get_closing_signed_broadcast!(nodes[0].node, nodes[1].node.get_our_node_id());
	nodes[1].node.handle_closing_signed(&nodes[0].node.get_our_node_id(), &closing_signed_2_node_0.unwrap());
	...
	// create_channel test
	let err1 = nodes[0].node.create_channel(unkown_public_key, 1_000_000, 500_000_000, 42, None);

	nodes[1].node.handle_open_channel(&unkown_public_key, InitFeatures::known(), &open_channel);
	closure_test(&nodes[1], unkown_public_key, open_channel.temporary_channel_id);

	nodes[0].node.handle_accept_channel(&unkown_public_key, InitFeatures::known(), &accept_channel);
	closure_test(&nodes[0], unkown_public_key, open_channel.temporary_channel_id);

	let err1 = nodes[0].node.accept_inbound_channel(&open_channel.temporary_channel_id, &unkown_public_key, 42);

	nodes[1].node.handle_funding_created(&unkown_public_key, &funding_created_msg);
	closure_test(&nodes[1], unkown_public_key, temporary_channel_id);

	nodes[0].node.handle_funding_signed(&unkown_public_key, &funding_signed);
	closure_test(&nodes[0], unkown_public_key, funding_signed.channel_id);

	nodes[0].node.handle_channel_ready(&unkown_public_key, &nodes_1_channel_ready);
	closure_test(&nodes[0], unkown_public_key, nodes_0_channel_ready.channel_id);

	nodes[1].node.handle_announcement_signatures(&unkown_public_key, &nodes_0_announcement_sigs);
	closure_test(&nodes[1], unkown_public_key, nodes_0_channel_ready.channel_id);

	let err1 = nodes[0].node.close_channel(&nodes_0_channel_ready.channel_id, &unkown_public_key);

	let err2 = nodes[0].node.force_close_channel(&nodes_0_channel_ready.channel_id, &unkown_public_key);

	nodes[0].node.handle_shutdown(&unkown_public_key, &InitFeatures::known(), &nodes_1_shutdown);
	closure_test(&nodes[0], unkown_public_key, nodes_0_channel_ready.channel_id);

	nodes[1].node.handle_closing_signed(&unkown_public_key, &closing_signed_node_0);
	closure_test(&nodes[1], unkown_public_key, nodes_0_channel_ready.channel_id);

	nodes[0].node.handle_channel_reestablish(&unkown_public_key, &reestablish_2[0]);
	closure_test(&nodes[0], unkown_public_key, nodes_0_channel_ready.channel_id);

	nodes[1].node.handle_update_add_htlc(&unkown_public_key, &update_add_htlc_msg);
	closure_test(&nodes[1], unkown_public_key, channel_id);
	...
	let close_msg_ev1 = nodes[0].node.get_and_clear_pending_msg_events();
	let close_msg_ev2 = nodes[1].node.get_and_clear_pending_msg_events();
	let close_ev1 = nodes[0].node.get_and_clear_pending_events();
	let close_ev2 = nodes[1].node.get_and_clear_pending_events();
	let a = 3;
}


#[test]
fn test_function_calls_with_unkown_counterparty_node() {
	let chanmon_cfg = create_chanmon_cfgs(4);
	let node_cfg = create_node_cfgs(4, &chanmon_cfg);
	let node_chanmgr = create_node_chanmgrs(4, &node_cfg, &[None, None, None, None]);
	let nodes = create_network(4, &node_cfg, &node_chanmgr);
	...
	// create_channel test
	let err1 = nodes[0].node.create_channel(unkown_public_key, 1_000_000, 500_000_000, 42, None);
	// TODO: ASSERT MISSUSE ERR

	// handle_open_channel test
	nodes[0].node.create_channel(nodes[1].node.get_our_node_id(), 1_000_000, 500_000_000, 42, None).unwrap();
	let open_channel = get_event_msg!(nodes[0], MessageSendEvent::SendOpenChannel, nodes[1].node.get_our_node_id());
	nodes[1].node.handle_open_channel(&unkown_public_key, InitFeatures::known(), &open_channel);
	...
	// funding_transaction_generated test
	nodes[0].node.create_channel(nodes[1].node.get_our_node_id(), 1_000_000, 500_000_000, 42, None).unwrap();
	let open_channel = get_event_msg!(nodes[0], MessageSendEvent::SendOpenChannel, nodes[1].node.get_our_node_id());
	nodes[1].node.handle_open_channel(&nodes[0].node.get_our_node_id(), InitFeatures::known(), &open_channel);
	let accept_channel = get_event_msg!(nodes[1], MessageSendEvent::SendAcceptChannel, nodes[0].node.get_our_node_id());
	nodes[0].node.handle_accept_channel(&nodes[1].node.get_our_node_id(), InitFeatures::known(), &accept_channel);
	let (temporary_channel_id, tx, _funding_output) = create_funding_transaction(&nodes[0], &nodes[1].node.get_our_node_id(), 1_000_000, 42);
	let err2 = nodes[0].node.funding_transaction_generated(&temporary_channel_id, &unkown_public_key, tx.clone());

	// handle_funding_created test
	nodes[0].node.funding_transaction_generated(&temporary_channel_id, &nodes[1].node.get_our_node_id(), tx.clone()).unwrap();
	let funding_created_msg = get_event_msg!(nodes[0], MessageSendEvent::SendFundingCreated, nodes[1].node.get_our_node_id());
	nodes[1].node.handle_funding_created(&unkown_public_key, &funding_created_msg);
	closure_test(&nodes[1], unkown_public_key, temporary_channel_id);

	// handle_funding_signed test
	nodes[0].node.create_channel(nodes[1].node.get_our_node_id(), 1_000_000, 500_000_000, 42, None).unwrap();
	let open_channel = get_event_msg!(nodes[0], MessageSendEvent::SendOpenChannel, nodes[1].node.get_our_node_id());
	nodes[1].node.handle_open_channel(&nodes[0].node.get_our_node_id(), InitFeatures::known(), &open_channel);
	let accept_channel = get_event_msg!(nodes[1], MessageSendEvent::SendAcceptChannel, nodes[0].node.get_our_node_id());
	...
	connect_blocks(&nodes[1], CHAN_CONFIRM_DEPTH - 1);
	let nodes_0_channel_ready = get_event_msg!(nodes[0], MessageSendEvent::SendChannelReady, nodes[1].node.get_our_node_id());
	nodes[0].node.handle_channel_ready(&unkown_public_key, &get_event_msg!(nodes[1], MessageSendEvent::SendChannelReady, nodes[0].node.get_our_node_id()));
	closure_test(&nodes[0], unkown_public_key, nodes_0_channel_ready.channel_id);

	// handle_announcement_signatures test
	let tx = create_chan_between_nodes_with_value_init(&nodes[0], &nodes[1], 100000, 10001, InitFeatures::known(), InitFeatures::known());
	let conf_height = core::cmp::max(nodes[0].best_block_info().1 + 1, nodes[1].best_block_info().1 + 1);
	confirm_transaction_at(&nodes[0], &tx, conf_height);
	connect_blocks(&nodes[0], CHAN_CONFIRM_DEPTH - 1);
	...
	let msg_events = nodes[0].node.get_and_clear_pending_msg_events();
	assert_eq!(msg_events.len(), 1);
	let nodes_0_announcement_sigs = match msg_events[0] {
		MessageSendEvent::SendAnnouncementSignatures { ref node_id, ref msg } => {
			assert_eq!(*node_id, nodes[1].node.get_our_node_id());
			msg.clone()
		},
	...
	// close_channel test
	let chan = create_unannounced_chan_between_nodes_with_value(&nodes, 0, 1, 100000, 10001, InitFeatures::known(), InitFeatures::known());
	let err1 = nodes[0].node.close_channel(&chan.0.channel_id, &unkown_public_key);
	// force_close_channel test
	let err2 = nodes[0].node.force_close_channel(&chan.0.channel_id, &unkown_public_key);

	// handle_shutdown test
	nodes[0].node.close_channel(&chan.0.channel_id, &nodes[1].node.get_our_node_id()).unwrap();
	nodes[1].node.handle_shutdown(&unkown_public_key, &InitFeatures::known(), &get_event_msg!(nodes[0], MessageSendEvent::SendShutdown, nodes[1].node.get_our_node_id()));
	closure_test(&nodes[1], unkown_public_key, chan.0.channel_id);

	// handle_closing_signed test
	let chan = create_unannounced_chan_between_nodes_with_value(&nodes, 0, 1, 100000, 10001, InitFeatures::known(), InitFeatures::known());
	nodes[0].node.close_channel(&chan.0.channel_id, &nodes[1].node.get_our_node_id()).unwrap();
	nodes[1].node.handle_shutdown(&nodes[0].node.get_our_node_id(), &InitFeatures::known(), &get_event_msg!(nodes[0], MessageSendEvent::SendShutdown, nodes[1].node.get_our_node_id()));
	let events = nodes[1].node.get_and_clear_pending_msg_events();
	...
	// handle_channel_reestablish test. We use nodes[2] and nodes[1] as nodes[0] and nodes[1]
	// already have multiple channels setup between them.
	let chan = create_unannounced_chan_between_nodes_with_value(&nodes, 2, 1, 100000, 10001, InitFeatures::known(), InitFeatures::known());
	...
	let reestablish_1 = get_chan_reestablish_msgs!(nodes[2], nodes[1]);
	nodes[1].node.peer_connected(&nodes[2].node.get_our_node_id(), &msgs::Init { features: InitFeatures::empty(), remote_network_address: None });
	let reestablish_2 = get_chan_reestablish_msgs!(nodes[1], nodes[2]);
	nodes[2].node.handle_channel_reestablish(&unkown_public_key, &reestablish_2[0]);
	closure_test(&nodes[2], unkown_public_key, chan.0.channel_id);

	// handle_update_add_htlc test
	let (route, our_payment_hash, _, our_payment_secret) = get_route_and_payment_hash!(nodes[0], nodes[1], 1000000);
	nodes[0].node.send_payment(&route, our_payment_hash, &Some(our_payment_secret)).unwrap();
	check_added_monitors!(nodes[0], 1);
	let updates = get_htlc_update_msgs!(nodes[0], nodes[1].node.get_our_node_id());
	nodes[1].node.handle_update_add_htlc(&unkown_public_key, &updates.update_add_htlcs[0]);
	closure_test(&nodes[1], unkown_public_key, updates.update_add_htlcs[0].channel_id);

	// handle_commitment_signed test
	nodes[1].node.handle_commitment_signed(&unkown_public_key, &updates.commitment_signed);
	closure_test(&nodes[1], unkown_public_key, updates.update_add_htlcs[0].channel_id);

	create_unannounced_chan_between_nodes_with_value(&nodes, 2, 3, 100000, 10001, InitFeatures::known(), InitFeatures::known());
	let (route, our_payment_hash, payment_preimage, our_payment_secret) = get_route_and_payment_hash!(nodes[2], nodes[3], 1000000);
	nodes[2].node.send_payment(&route, our_payment_hash, &Some(our_payment_secret)).unwrap();
	check_added_monitors!(nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], nodes[3].node.get_our_node_id());
	nodes[3].node.handle_update_add_htlc(&nodes[2].node.get_our_node_id(), &updates.update_add_htlcs[0]);
	commitment_signed_dance!(nodes[3], nodes[2], updates.commitment_signed, false, true);

	nodes[3].node.claim_funds(payment_preimage);
	check_added_monitors!(nodes[3], 1);
	expect_payment_claimed!(nodes[3], our_payment_hash, 1000000);

	{
		let mut added_monitors = nodes[0].chain_monitor.added_monitors.lock().unwrap();
		let a = added_monitors.len();
		added_monitors.clear();
	}

	{
		let mut added_monitors = nodes[1].chain_monitor.added_monitors.lock().unwrap();
		let a = added_monitors.len();
		let close_msg_ev = nodes[0].node.get_and_clear_pending_msg_events();
		added_monitors.clear();
	}
